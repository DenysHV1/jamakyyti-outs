export const cookiePolicy = () => {
  ...
  if (JSON.parse(localStorage.getItem('jamakyyti-status'))) {
  ...
  btnEl1.addEventListener('click', () => {
    modalEl.style.display = 'none';
    localStorage.setItem('jamakyyti-status', JSON.stringify(true));
  });

  btnEl2.addEventListener('click', () => {
    modalEl.style.display = 'none';
    localStorage.setItem('jamakyyti-status', JSON.stringify(false));
  });
};
